...
public class Circuit implements Copyable<Circuit> {
    ...
    /**
     * Updates the wire colors based on the inputs and outputs they are connected to.
     */
    public void updateWireColors() {
        for (Wire w : wires)
            w.setColor(null);

        java.awt.Color defaultWireColor = Keys.WIRE_COLOR.getDefault();

        try {
            NetList nl = new NetList(this);
            for (VisualElement ve : visualElements) {
                for (Pin p : ve.getPins()) {
                    Net n = nl.getNetOfPos(p.getPos());
                    if (n != null)
                        n.add(p);
                }
            }
            ...
            for (Net n : nl) {
                ...
                // check if one of the wires has a custom color
                for (Wire w : n.getWires()) {
                    if (w.getCustomColor() != null) {
                        color = w.getCustomColor();
                        break;
                    }
                }

                if (color == null) {
                    // check if one of the pins has a wire color set
                    for (Pin p : n.getPins()) {
                        VisualElement ve = p.getVisualElement();
                        if (ve != null && ve.getElementAttributes().contains(Keys.WIRE_COLOR)) {
                            java.awt.Color c = ve.getElementAttributes().get(Keys.WIRE_COLOR);
                            if (c != null && !c.equals(defaultWireColor)) {
                                color = c;
                                break;
                            }
                        }
