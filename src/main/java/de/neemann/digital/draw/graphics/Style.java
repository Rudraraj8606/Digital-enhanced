...
public final class Style {
    ...
    public static Style getWireStyle(Value value) {
        ...
        java.awt.Color customColor = value.getColor();
        if (customColor != null && !customColor.equals(Keys.WIRE_COLOR.getDefault())) {
            if (value.isHighZ()) return WIRE_HIGHZ;
            if (value.getBits() > 1)
                return WIRE.deriveColor(customColor);

            if (value.getValue() == 0)
                return WIRE_LOW.deriveColor(brighten(customColor));
            else
                return WIRE_HIGH.deriveColor(darken(customColor));
        }

        if (value.getBits() > 1) return WIRE;
    ...
    private static Color brighten(Color c) {
        ...
        return Color.getHSBColor(hsb[0], hsb[1] * 0.5f, Math.min(1.0f, hsb[2] * 1.2f));
    }

    private static Color darken(Color c) {
        float[] hsb = Color.RGBtoHSB(c.getRed(), c.getGreen(), c.getBlue(), null);
        return Color.getHSBColor(hsb[0], Math.min(1.0f, hsb[1] * 1.2f), hsb[2] * 0.7f);
    }
