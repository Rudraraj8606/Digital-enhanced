...
public class NetList implements Iterable<Net> {
    ...
    private void add(Wire w) {
        Net n1 = getNetOfPos(w.p1);
        Net n2 = getNetOfPos(w.p2);
        ...
        if (n1 == null && n2 == null) {
            ...
            // wire connects two points of the same net, nothing to do
            return;
        } else {
            // wire connects two different nets, merge them
            n1.addAllPointsFrom(n2);
            n1.tryMerge(w);
            netList.remove(n2);
        }
    }
